...
namespace SoulRemake
{
    public class ActorController : MonoBehaviour
    {
        ...
        private void Update()
        {
            var runFactor = (input.canRun ? runMultiplier : 1f);
            if (!camCtrl.lockState)
            {
                anim.SetFloat(anim_forward, input.dMag * runFactor);
                anim.SetFloat(anim_right, 0);
            }
            else
            {
                var localVec=transform.InverseTransformVector(input.dVec);
                anim.SetFloat(anim_forward, localVec.z*runFactor);
                anim.SetFloat(anim_right,localVec.x*runFactor);
            }
            ...
            else
            {
                ...
                if (!lockPlanar)
                {
                    planarVec=input.dVec*walkSpeed * (input.canRun ? runMultiplier : 1f);
                }
            }
